...
func init() {
	...
	flag.StringVar(&cfg.Package, "package", "", "package path for the generated file. Blank for the first loaded package")
...
func newConcrete(p *packages.Package, x, sel string, m Methoder, pos token.Pos) *Concrete {
	c := &Concrete{Name: sel, PackageName: x, PackagePath: m.Obj().Pkg().Path(), FoundIn: p.Name, FoundInPath: p.PkgPath, AllMethods: make([]Method, 0, m.NumMethods()), Used: map[string]struct{}{}, Pos: pos}
...
func writeTo(c *Concrete, w io.Writer, cfg config) error {
	...
	if cfg.Package == "" {
		...
		pkgName, pkgPath = c.FoundIn, c.FoundInPath
	} else {
		pkgs, err := load(cfg.Package)
		if err != nil {
			return err
		}
		pkgName, pkgPath = pkgs[0].Name, pkgs[0].PkgPath
	...
	for _, m := range c.AllMethods {
		...
		writeParams(m.Params, &b, pkgName, pkgRewrites)
		b.WriteString(")")
		...
		if len(m.Returns) > 0 {
			...
			writeParams(m.Returns, &b, pkgName, pkgRewrites)
...
func writeParams(params []Param, w io.Writer, dstPkg string, pkgRewrites map[string]string) {
	for i, p := range params {
		...
		if len(kind) > identStart+5 && kind[identStart:identStart+5] == "chan " {
			identStart += 5
		}
