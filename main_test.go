package main

import (
	"bytes"
	"regexp"
	"strings"
	"testing"

	"github.com/stretchr/testify/assert"
)

func Test_locateType_write(t *testing.T) {
	type args struct {
		selector string
		pattern  string
	}
	tests := []struct {
		name    string
		args    args
		config  config
		want    string
		wantErr bool
	}{
		{name: "unused and embedded", args: args{selector: "bar.Bar", pattern: "./testdata"}, want: constBarUsage},
		{name: "ignore method used by the implementation", args: args{selector: "testdata.Baz", pattern: "./testdata"}, want: someBazUsage},
		{name: "directional chan", args: args{selector: "bar.Streamer", pattern: "./testdata"}, want: someStreamerUsage},
		{name: "directional chan 2", args: args{selector: "bar.StreamConsumer", pattern: "./testdata"}, want: someStreamConsumerUsage},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			packages, err := load(tt.args.pattern)
			if err != nil {
				t.Fatal(err)
			}

			c, err := locateType(tt.args.selector, packages[0])
			if (err != nil) != tt.wantErr {
				t.Errorf("locateType() error = %v, wantErr %v", err, tt.wantErr)
			}

			var got bytes.Buffer
			err = writeTo(c, &got, tt.config)
			assert.NoError(t, err)

			assert.Equal(t, normalizeComment(tt.want), normalizeComment(got.String()))

		})
	}
}

var re = regexp.MustCompile(`generated by .*interface-extractor.*; DO NOT EDIT.`)

func normalizeComment(in string) string {
	return re.ReplaceAllString(strings.TrimSpace(in), "generated by interface-extractor !DO NOT EDIT!")
}

const (
	constBarUsage = `
// generated by interface-extractor.test -test.timeout=10s -test.timeout=10s; DO NOT EDIT.

package testdata

type Barer interface {
	Const() int
	EmbeddedMethod(i int) string
}
	`
	someBazUsage = `
// generated by interface-extractor.test -test.timeout=10s -test.timeout=10s; DO NOT EDIT.

package testdata

type Bazer interface {
	SomeMethod(i int)
}
	`

	someStreamerUsage = `
// generated by interface-extractor.test -test.timeout=10s -test.timeout=10s; DO NOT EDIT.

package testdata

import (
	"github.com/urandom/interface-extractor/testdata/bar"
)

type Streamer interface {
	Stream() <-chan bar.Alpha
}
	`

	someStreamConsumerUsage = `
// generated by interface-extractor.test -test.timeout=10s -test.timeout=10s; DO NOT EDIT.

package testdata

import (
	"github.com/urandom/interface-extractor/testdata/bar"
)

type StreamConsumer interface {
	Consume(_ <-chan bar.Alpha)
}
	`
)
